...
with torch.no_grad():
    for tree in range(num_trees):
        ...
        for step in range(num_frames_to_generate):
            ...
            #print(step, num_frames_to_generate)

            if step == num_frames_to_generate-1:
                generated_trees.append(next_frame)

        if (tree + 1) % 10 == 0:
            print(f"Generated {tree+1}/{num_trees} trees")
...
num_frames = len(generated_trees)


for tree in range(num_trees):
    frame_vec = generated_trees[tree].cpu().numpy()
    ...
    filename = os.path.join(output_dir, f"tree_{tree}.txt")
    with open(filename, 'w') as f:
        f.write(content)
    
    if (tree + 1) % 50 == 0:
        print(f"Saved {i+1}/{num_trees} trees")
